...
class InsightNode:
    ...
    def _build_situation_summary(cls, top_topics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        case_candidate_count = sum(1 for t in top_topics if isinstance(t, dict) and t.get("case_candidate"))
        highest_topic = None
        if top_topics:
            highest_topic = max(top_topics, key=lambda t: float(t.get("risk_score") or 0))
        highest_risk_topic = str(highest_topic.get("topic") or "") if isinstance(highest_topic, dict) else ""
        highest_risk_level = str(highest_topic.get("risk_level") or "低风险") if isinstance(highest_topic, dict) else "低风险"
        overview = (
...
"""
        ...
        try:
            ...
            if isinstance(analysis_result, dict):
                ...
                analysis_result = {"top_topics": [], "summary": "模型返回结果格式异常", "situation_summary": self._build_situation_summary([])}
