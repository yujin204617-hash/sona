"""测试脚本：调用 region_stats 工具，统计 IP属地 省份 TopN 并输出 JSON。"""
...
from tools.region_stats import region_stats  # noqa: E402
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 region_stats 工具，输出 IP属地 省份 TopN 统计结果。")
    parser.add_argument(
        "--data",
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    try:
        ...
        # 若命中“IP属地”，则显式指定，以提升识别稳定性
        forced_col = None
        normalized_headers = [str(h).strip() for h in headers]
        if "IP属地" in normalized_headers:
            forced_col = "IP属地"
        ...
        if forced_col:
            print(f"[IP属地列] 使用指定列: {forced_col}")
            invoke_params["ipLocationColumn"] = forced_col
        result = region_stats.invoke(invoke_params)

        if isinstance(result, str):
            parsed = json.loads(result)
            now = datetime.now().strftime("%Y%m%d_%H%M%S")
            out_name = f"region_stats_result_{now}.json"
